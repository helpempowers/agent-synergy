...
class ApiClient {
  ...
  async chatWithAgent(agentId: string, message: string, conversationType: string = 'custom', metadata?: Record<string, any>): Promise<Conversation> {
  ...
  async addMessage(conversationId: string, messageData: { role: string; content: string; metadata?: Record<string, any> }): Promise<{ message: string; message_id: string }> {
